...
type Vehicle = { id:number; name:string; passenger_capacity:number|null };
...
type RatePayload = { source?:string; updated_at?:string|null; currency?:string; vehicle_types?:Vehicle[]; rate_cards?:RateCard[] };
...
export default function Home(){
  ...
  return <main>
    ...
    <section className="section cream" id="fleet"><div className="title"><p>THE A3 FLEET</p><h2>Premium comfort for every group.</h2></div>
      <div className="grid three fleet">{vehicles.map(vehicle=><article key={vehicle.id}><div className="vehicle">◆<small>CHAUFFEUR VEHICLE</small></div><h3>{vehicle.name}</h3><p>{vehicle.passenger_capacity?`Suitable for up to ${vehicle.passenger_capacity} passengers, subject to luggage requirements.`:"Comfortable private transport for your journey."}</p><a href="/book">Book this vehicle →</a></article>)}</div>
    </section>
